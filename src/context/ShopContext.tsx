...
import { getProductCode, getProductUrl } from '../utils/productCode';
import {
  signInWithPopup,
...
/**
 * Match a product by code, slug, id, or partial match from URL
 */
export function findMatchingProduct(targetCode: string, productList: Product[]): Product | undefined {
  if (!targetCode || !productList || productList.length === 0) return undefined;

  const clean = targetCode.trim().toLowerCase();
  const digitsOnly = clean.replace(/\D/g, '');
  const cleanNoPrefix = clean.replace(/^fa[-_]?/i, '');

  // 1. Exact match on productCode, ID, or slug (case-insensitive)
  let found = productList.find(p => {
    const pCode = (p.productCode || '').trim().toLowerCase();
    const pId = (p.id || '').trim().toLowerCase();
    const pSlug = (p.slug || '').trim().toLowerCase();
    const dynCode = getProductCode(p).toLowerCase();
    return pCode === clean || dynCode === clean || pId === clean || pSlug === clean;
  });
  if (found) return found;

  // 2. Match without "FA-" prefix (e.g. user or external link omitted "FA-" or formatted as "84920")
  found = productList.find(p => {
    const pCodeNoPrefix = (p.productCode || '').trim().toLowerCase().replace(/^fa[-_]?/i, '');
    const dynCodeNoPrefix = getProductCode(p).toLowerCase().replace(/^fa[-_]?/i, '');
    return (
      (pCodeNoPrefix && pCodeNoPrefix === cleanNoPrefix) ||
      (dynCodeNoPrefix && dynCodeNoPrefix === cleanNoPrefix)
    );
  });
  if (found) return found;

  // 3. Match by numeric digits if at least 4 digits
  if (digitsOnly.length >= 4) {
    found = productList.find(p => {
      const pCodeDigits = (p.productCode || '').replace(/\D/g, '');
      const dynDigits = getProductCode(p).replace(/\D/g, '');
      const idDigits = (p.id || '').replace(/\D/g, '');
      return (
        (pCodeDigits && (pCodeDigits.endsWith(digitsOnly) || digitsOnly.endsWith(pCodeDigits))) ||
        (dynDigits && (dynDigits.endsWith(digitsOnly) || digitsOnly.endsWith(dynDigits))) ||
        (idDigits && (idDigits.endsWith(digitsOnly) || digitsOnly.endsWith(idDigits)))
      );
    });
    if (found) return found;
  }

  // 4. Fuzzy match on product name or slug
  found = productList.find(p => {
    const pSlug = (p.slug || '').toLowerCase();
    const pName = (p.name || '').toLowerCase();
    return (
      (pSlug && (clean.includes(pSlug) || pSlug.includes(clean))) ||
...
export const ShopProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [products, setProducts] = useState<Product[]>(() => {
    try {
      const saved = localStorage.getItem('fa_catalog_products');
      if (saved) {
  ...
  const [categories, setCategories] = useState<Category[]>(() => {
    try {
      const saved = localStorage.getItem('fa_catalog_categories');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed;
  ...
  // View & Modals: initialize view from URL so direct links immediately display the product view
  const [activeView, setActiveView] = useState<'home' | 'catalog' | 'admin' | 'product'>(() => {
    if (typeof window !== 'undefined') {
      const pathname = window.location.pathname;
      const hash = window.location.hash;
      if (pathname.match(/\/product\/[^/?#]+/i) || hash.match(/#\/?product\/[^/?#]+/i)) {
        return 'product';
      }
  ...
  // Immediately initialize quickViewProduct from initial or cached catalog if URL is /product/CODE
  const [quickViewProduct, setQuickViewProductState] = useState<Product | null>(() => {
    if (typeof window !== 'undefined') {
      const pathname = window.location.pathname;
      const hash = window.location.hash;
      const pathMatch = pathname.match(/\/product\/([^/?#]+)/i);
      const hashMatch = hash.match(/#\/?product\/([^/?#]+)/i);
      const code = (pathMatch && pathMatch[1]) || (hashMatch && hashMatch[1]);
      if (code) {
        const initialProds = (() => {
          try {
            const saved = localStorage.getItem('fa_catalog_products');
        ...
        return findMatchingProduct(decodeURIComponent(code), initialProds) || null;
      }
    }
    return null;
  });
  ...
  // Helper to extract product code from URL (path: /product/CODE or hash: #/product/CODE)
  const getProductCodeFromUrl = useCallback((): string | null => {
    try {
      const pathname = window.location.pathname;
      const pathMatch = pathname.match(/\/product\/([^/?#]+)/i);
  ...
  const openProductDetail = useCallback((product: Product) => {
    ...
    const targetUrl = `/product/${encodeURIComponent(code)}`;
    if (window.location.pathname !== targetUrl) {
      window.history.pushState({ type: 'product', code }, '', targetUrl);
    }
  ...
  const handleSetActiveView = useCallback((view: 'home' | 'catalog' | 'admin' | 'product') => {
    setActiveView(view);
    if (view === 'home') {
      if (window.location.pathname.startsWith('/product') || window.location.hash.startsWith('#admin')) {
  ...
  // Listen to browser popstate, hashchange, and synchronize URL on initial load & products updates
  useEffect(() => {
    let isCancelled = false;

    const syncRouteWithUrl = async () => {
      const targetCode = getProductCodeFromUrl();
      if (targetCode) {
        // 1. Try matching with current products in state
        let matched = findMatchingProduct(targetCode, products);

        // 2. If not found, try localStorage cache
        if (!matched) {
          try {
            const saved = localStorage.getItem('fa_catalog_products');
            if (saved) {
              ...
              if (Array.isArray(parsed)) {
                matched = findMatchingProduct(targetCode, parsed);
              }
            }
          } catch {}
        }

        // 3. If not found, try initial fallback products
        if (!matched) {
          matched = findMatchingProduct(targetCode, INITIAL_PRODUCTS);
        }

        if (matched) {
          if (!isCancelled) {
            setQuickViewProductState(matched);
            setActiveView('product');
          }
        } else {
          // 4. If still not matched, fetch fresh products from database/API
          try {
            const freshProducts = await api.getProducts();
            if (!isCancelled && freshProducts && freshProducts.length > 0) {
              setProducts(freshProducts);
              const liveMatched = findMatchingProduct(targetCode, freshProducts);
              if (liveMatched) {
                setQuickViewProductState(liveMatched);
                setActiveView('product');
              }
            }
          } catch (e) {
            console.warn('Direct product route lookup error:', e);
          }
        }
      } else if (window.location.hash === '#admin' || window.location.pathname.includes('/admin')) {
        if (!isCancelled) setActiveView('admin');
      } else if (window.location.pathname === '/' || window.location.pathname === '') {
        // If we were on product detail and user clicked browser Back to '/', show home
        if (!isCancelled) setActiveView(prev => (prev === 'product' ? 'home' : prev));
      }
    };
  ...
  }, [products, getProductCodeFromUrl]);
