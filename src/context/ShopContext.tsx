...
import { Product, Category, District, CartItem, Order, User } from '../types';
...
interface ShopContextType {
  ...
  activeView: 'home' | 'catalog' | 'admin';
  setActiveView: (view: 'home' | 'catalog' | 'admin') => void;
...
export const ShopProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [wishlist, setWishlist] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('fa_wishlist');
      return saved ? JSON.parse(saved) : [];
    } catch {
      return [];
    }
  ...
  // View & Modals
  const [activeView, setActiveView] = useState<'home' | 'catalog' | 'admin'>('home');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('fa_wishlist', JSON.stringify(wishlist));
    } catch (e) {
      console.error(e);
    }
  }, [wishlist]);
  ...
  useEffect(() => {
    ...
    // Ensure Cloud Firestore has categories and products seeded for universal access
    seedCatalogToCloudIfEmpty(INITIAL_CATEGORIES, INITIAL_PRODUCTS);

    // Live Real-Time Listener: Automatically syncs new products to ALL visitors without clicking sync
    ...
    // Live Real-Time Listener: Automatically syncs new categories to ALL visitors without clicking sync
  ...
  }, [reloadDistricts, reloadCategories, reloadProducts]);
