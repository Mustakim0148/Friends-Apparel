    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-stone-900 mb-2">
          {urlCode ? `Looking for Product ${decodeURIComponent(urlCode)}...` : 'No Product Selected'}
        </h2>
        <p className="text-stone-500 text-sm max-w-md mx-auto mb-6">
          {urlCode
            ? `If this product code is correct, it will load shortly. Otherwise, you can explore our latest collection.`
            : 'Select any product from the catalog to see its full specifications, images, and live code.'}
        </p>
        <button
          onClick={() => setActiveView('catalog')}
          className="px-6 py-2.5 rounded-full bg-rose-800 text-white font-semibold text-sm hover:bg-rose-900 transition-colors cursor-pointer shadow-md"
        >
          View All Products
        </button>
      </div>
