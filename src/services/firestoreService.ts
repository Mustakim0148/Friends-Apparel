...
/**
 * Push Subscriptions Collection for Web Push
 */
const PUSH_SUBS_COLLECTION = 'push_subscriptions';

export async function savePushSubscriptionToCloud(subscription: {
  endpoint: string;
  keys?: { p256dh?: string; auth?: string };
  userAgent?: string;
}): Promise<void> {
  try {
    const key = btoa(subscription.endpoint).replace(/[/+=]/g, '_').slice(-60);
    const ref = doc(db, PUSH_SUBS_COLLECTION, key);
    await setDoc(ref, cleanForFirestore({
      ...subscription,
      updatedAt: new Date().toISOString(),
    }), { merge: true });
    console.log('[Cloud Firestore] Push subscription synced.');
  } catch (err) {
    console.warn('Failed to save push subscription to Firestore:', err);
  }
}

export async function removePushSubscriptionFromCloud(endpoint: string): Promise<void> {
  try {
    const key = btoa(endpoint).replace(/[/+=]/g, '_').slice(-60);
    const ref = doc(db, PUSH_SUBS_COLLECTION, key);
    await deleteDoc(ref);
  } catch (err) {
    console.warn('Failed to remove push subscription from Firestore:', err);
  }
}
